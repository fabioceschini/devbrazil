...
DESCRIPTIONS = {
    "pt": "DevBrazil: Desenvolvimento de aplicativos nativos, rápidos e independentes para Windows. Soluções focadas em alta performance e total privacidade dos seus dados.",
    "en": "DevBrazil: Fast, independent, native Windows application development. Software solutions focused on high performance, productivity, and total data privacy.",
    "es": "DevBrazil: Desarrollo de aplicaciones nativas, rápidas e independientes para Windows. Soluciones enfocadas en alto rendimiento y total privacidad de sus datos.",
    "fr": "DevBrazil : Développement d'applications natives, rapides et indépendantes pour Windows. Solutions axées sur la haute performance et la confidentialité des données.",
    "de": "DevBrazil: Entwicklung nativer, schneller und unabhängiger Windows-Anwendungen. Softwarelösungen mit Fokus auf Hohe Leistung und vollen Datenschutz.",
}
...
def set_meta_description(soup, language):
    """
    Inserts (or replaces) <meta name="description"> with the
    ...
    # Remove any existing description tag so generation is repeatable.
    existing = soup.head.find("meta", attrs={"name": "description"})

    if existing:
        existing.decompose()

    meta_desc = DESCRIPTIONS.get(language)
    ...
    meta_tag = soup.new_tag(
        "meta",
        attrs={"name": "description", "content": meta_desc}
    )
...
def generate_page(source_html, language, output_file, nested):
    ...
    set_meta_description(soup, language)
